...
function PlayerPotWalkState:update(dt)
    ...
    if love.keyboard.wasPressed('space') then
        table.insert(self.dungeon.currentRoom.projectiles, Projectile({
            x = self.entity.direction == 'left' and self.entity.x-self.entity.lifted.width or self.entity.x,
            y = self.entity.direction == 'down' and self.entity.y+self.entity.lifted.height or self.entity.y,
            direction = self.entity.direction,
            texture = self.entity.lifted.texture,
            range = 5*TILE_SIZE,
            vel = 8*TILE_SIZE,
            frame = self.entity.lifted.frame
        }))
        self.entity.lifted = nil
