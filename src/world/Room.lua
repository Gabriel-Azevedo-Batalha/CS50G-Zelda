...
function Room:generateObjects()
    ...
    pot.onCollide = function()
        if pot.state == 'unlifted' and love.keyboard.isDown('space') then
            self.player.lifted = pot
            pot.state = 'lifted'
            pot.visible = false
            self.player:changeState('pot-idle')
        elseif pot.state == 'lifted' then
            pot.solid = false
        end
    end   
...
function Room:update(dt)
    ...
    for i = #self.entities, 1, -1 do
        ...
        if entity.health <= 0 and not entity.dead then
            ...
            if math.random(10) == 10 then
                local heart = GameObject(GAME_OBJECT_DEFS['heart'], entity.x+4, entity.y+4)
                heart.onCollide = function()
                    if heart.state == 'dropped' then
                        heart.state = 'picked'
                        self.player:heal(2)
                        heart.visible = false
                    end
    ...
    for k, projectile in pairs(self.projectiles) do
        if projectile.inGame then
            projectile:update(dt)
            for i = #self.entities, 1, -1 do
                local entity = self.entities[i]
                entity.bumped = true
                if entity:collides(projectile) then
                    projectile.inGame = false
                    entity:damage(1)
